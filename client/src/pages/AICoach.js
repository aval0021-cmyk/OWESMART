...
const AICoach = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col">
      {/* Header - Sticky */}
      <div className="sticky top-0 z-50 bg-slate-800 px-6 py-4 border-b border-slate-700 shadow-lg">
        <div className="flex items-center justify-between max-w-4xl mx-auto">
          <div>
            <h1 className="text-white text-xl font-bold">AI Debt Coach</h1>
            <p className="text-slate-400 text-sm">Ask me anything about debt management</p>
          </div>
          <button
            onClick={() => navigate('/dashboard')}
            className="bg-slate-700 hover:bg-slate-600 text-white px-4 py-2 rounded-lg transition flex items-center gap-2 shadow-md hover:shadow-lg"
          >
            ...
            <div className="max-w-md mx-auto space-y-2">
              ...
              {quickQuestions.map((question, index) => (
                <button
                  ...
                  className="w-full bg-slate-800 hover:bg-slate-700 text-white px-4 py-3 rounded-xl text-left transition border border-slate-700"
                >
                  {question}
                </button>
              ))}
      ...
      <div className="sticky bottom-0 z-40 bg-slate-800 border-t border-slate-700 px-6 py-4 shadow-lg">
        <form onSubmit={sendMessage} className="flex gap-3 max-w-4xl mx-auto">
          <input
            ...
            className="flex-1 bg-slate-700 text-white px-4 py-3 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-500 placeholder-slate-400"
            disabled={loading}
          />
          <button
            type="submit"
            disabled={loading || !inputMessage.trim()}
            className="bg-blue-500 text-white px-6 py-3 rounded-full hover:bg-blue-600 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
          >
